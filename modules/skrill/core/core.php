...
class SkrillPaymentCore
{
    ...
    /**
     * @var skrillVersionControllApi
     */
    protected static $skrillVersionControllApi = 'http://81.169.251.23:9097/sendVersion';
     ...
     */
    public static $unallowedCountries = [
    ...
    public static $paymentMethods = [
        ...
        'MAE' => [
            'name' => 'Maestro',
            'allowedCountries' => ['GBR', 'ESP', 'IRL', 'AUT'],
        ],
        'GCB' => [
            'name' => 'Carte Bleue by Visa',
            'allowedCountries' => ['FRA'],
        ],
        'DNK' => [
            'name' => 'Dankort by Visa',
            'allowedCountries' => ['DNK'],
        ],
        'PSP' => [
            'name' => 'PostePay by Visa',
            'allowedCountries' => ['ITA'],
        ],
        'CSI' => [
        ...
        'GIR' => [
            'name' => 'Giropay',
            'allowedCountries' => ['DEU'],
        ],
        'SFT' => [
            'name' => 'Sofort',
            'allowedCountries' => ['DEU', 'AUT', 'BEL', 'NLD', 'ITA', 'FRA', 'POL', 'HUN', 'SVK', 'CZE', 'GBR'],
        ],
        'EBT' => [
            'name' => 'Nordea Solo',
            'allowedCountries' => ['SWE'],
        ],
        'IDL' => [
            'name' => 'iDEAL',
            'allowedCountries' => ['NLD'],
        ],
        'NPY' => [
            'name' => 'EPS (Netpay)',
            'allowedCountries' => ['AUT'],
        ],
        'PLI' => [
            'name' => 'POLi',
            'allowedCountries' => ['AUS'],
        ],
        'PWY' => [
            'name' => 'Przelewy24',
            'allowedCountries' => ['POL'],
        ],
        'EPY' => [
            'name' => 'ePay.bg',
            'allowedCountries' => ['BGR'],
        ],
        'GLU' => [
            'name' => 'Trustly',
            'allowedCountries' => [
    ...
    /**
     * send version controll data
     *
     * @param array $fields
     *
     * @return string
     */
    public static function sendVersionControll($fields)
    {
        $field = '{
            "shop_url": "' . $fields['shop_url'] . '",
            "ip_address": "' . $fields['ip_address'] . '",
            "shop_system": "' . $fields['shop_system'] . '",
            "merchant_id": "' . $fields['merchant_id'] . '",
            "shop_version": "' . $fields['shop_version'] . '"
        }';

        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, self::$skrillVersionControllApi);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl, CURLINFO_HEADER_OUT, true);
