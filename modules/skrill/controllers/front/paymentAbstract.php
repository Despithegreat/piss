...
class SkrillPaymentAbstractModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        try {
            PrestaShopLogger::addLog('send version controll to skrill', 1, null, 'Cart', $cart->id, true);
            $shopSettings = $this->getSkrillSettings();

            $versionControllParams = [];
            $versionControllParams['shop_url'] = Tools::getHttpHost(true) . __PS_BASE_URI__;
            $versionControllParams['ip_address'] = Tools::getRemoteAddr();
            $versionControllParams['shop_system'] = 'Prestashop';
            $versionControllParams['merchant_id'] = $shopSettings['merchant_id'];
            $versionControllParams['shop_version'] = _PS_VERSION_;

            PrestaShopLogger::addLog('Skrill - versionControllParams : ' . serialize($versionControllParams), 1, null, 'Cart', $cart->id, true);

            $versionControllResult = SkrillPaymentCore::sendVersionControll($versionControllParams);

            PrestaShopLogger::addLog('Skrill - version control response : ' . serialize($versionControllResult), 1, null, 'Cart', $cart->id, true);
        } catch (Exception $e) {
            PrestaShopLogger::addLog('fail to send version controll to skrill', 3, null, 'Cart', $cart->id, true);
        }
    ...
    private function getPostParameters()
    {
        ...
        if (preg_match('~[0-9]+~', $address->city)) {
        ...
        $postParameters['return_url'] = $contextLink->getModuleLink(
            'skrill',
            'validation',
            ['cart_id' => $cart->id, 'secure_key' => $customer->secure_key],
            true
        );
        ...
        $postParameters['cancel_url'] = $contextLink->getPageLink('order', true, null, ['step' => '3']);
